...
export const getFilteredVideos = createSelector(
   ...
   (videos, currentFilter, search) => {
      if (currentFilter && !search) {
         ...
         return videos.filter(video => video.status === currentFilter).filter(video => video.name.toLowerCase().includes(search.toLowerCase()))
      } else if (!currentFilter && search) {
         return videos.filter(video => video.name.toLowerCase().includes(search.toLowerCase()))
      } else {
         return videos
      }
   }
